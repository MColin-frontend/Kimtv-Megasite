...
export function BadgeLive({ label = "LIVE", className }: BadgeLiveProps) {
  return (
    <div className={cn("relative flex items-center gap-1.5 overflow-visible", className)}>
      ...
      <div className="rounded-6 shadow-live-red relative flex items-center gap-1.5 bg-red-600 px-2.5 py-1 max-sm:gap-1 max-sm:px-1.5 max-sm:py-0.5">
        <span className="relative flex size-2.5 shrink-0 max-sm:size-2">
          ...
          <span className="absolute inline-flex h-full w-full animate-ping rounded-full bg-white opacity-40 [animation-duration:1.2s] [animation-delay:0.2s]" />
